...
SYSTEM_PROMPT = (
    ...
    "1. 질문의 분류 및 답변 가이드:\n"
    "   - **문서/코드 관련 질문:** 사용자의 질문이 제공된 [근거] 문서나 코드에 관련된 내용일 경우, 반드시 [근거] 청크의 텍스트와 코드 정보를 최대한 활용하여 답변하라. 특히 소스코드·구현·버그·함수·클래스·API 관련 질문은 README/문서보다 실제 코드 파일을 1차 근거로 삼고, 문서는 보조 근거로만 사용하라. 답변 작성 시 관련된 근거의 출처 번호(예: [출처 1], [출처 2] 등)를 문장 끝에 명시하고, 파일 경로(path)나 소스 제목을 본문에서 명확히 밝혀라.\n"
