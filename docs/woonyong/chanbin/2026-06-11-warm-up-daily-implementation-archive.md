...
추가로 중요한 운영 판단이 있다. 찬빈은 완성하고 싶다는 욕심 때문에 AI를 무리하게 사용했고, 현재 코드의 전체 흐름을 충분히 이해하지 못하고 있을 가능성이 높다. 따라서 다음 단계는 기능 추가가 아니라 코드 소유권 회복이어야 한다.

현재 우선순위는 다음과 같이 바꾼다.

1. 새 기능 추가를 잠시 멈춘다.
2. 현재 구현된 로그인, 캘린더 조회, 작성 저장 흐름을 찬빈이 직접 설명한다.
3. 설명하지 못하는 부분은 작은 함수 단위로 다시 구현한다.
4. AI는 정답 생성 도구가 아니라 설명, 질문, 리뷰 도구로 제한한다.
...
- AI 사용이 코드 이해를 앞질렀을 가능성이 높다.
  - 완성된 결과물은 있지만, 찬빈이 각 흐름을 자기 말로 설명하지 못하면 유지보수와 디버깅이 어렵다.
  - 다음 기능을 붙이면 모르는 코드 위에 더 모르는 코드가 쌓인다.
  - 이 상태에서는 기능 완성도가 아니라 코드 소유권 회복을 먼저 확인해야 한다.

## 코드 소유권 회복 계획

현재 필요한 것은 더 많은 구현이 아니라 찬빈이 이미 작성된 코드를 자기 코드로 만드는 과정이다.

### 1. 기능 추가 중단

다음 기능인 상세 모달, 수정/삭제 UI는 잠시 멈춘다. 지금 바로 이어 붙이면 이해하지 못한 코드 위에 새 코드가 추가된다.

### 2. 설명 세션

찬빈이 아래 질문에 코드 없이 말로 답할 수 있어야 한다.

- 로그인하면 토큰은 어디에 저장되는가?
- 앱 시작 시 `/auth/me`는 왜 호출되는가?
- axios 요청 인터셉터는 어떤 역할을 하는가?
- 401 응답이 오면 어떤 흐름으로 로그아웃되는가?
- 캘린더는 언제 `/pages/calendar`를 호출하는가?
- `visibleYear`, `visibleMonth`, `selectedDate`는 각각 어떤 상태인가?
- 작성 모달 저장 버튼을 누르면 어떤 payload가 만들어지는가?
- 저장 성공 후 왜 캘린더를 다시 조회하는가?
- `Page`, `PageBlock`, `Tag`, `User`는 어떤 관계인가?
- 캘린더 조회와 일반 목록/검색/상세 조회의 권한 정책은 왜 문제가 되는가?

### 3. 작은 단위 재구현

설명하지 못하는 부분은 AI 도움 없이 작은 단위로 다시 작성한다.
...
- `normalizeTime`
- `createPage`
- `/pages/calendar`의 월 시작일/종료일 계산
- `calendarItems`를 `calendarEvents`로 바꾸는 변환
- 선택 날짜의 회의/회고 개수 계산
...
- “이 코드 흐름을 설명해줘”
- “내 설명이 맞는지 반례를 찾아줘”
- “이 함수의 입력과 출력을 표로 정리해줘”
- “정답 코드를 주지 말고 질문으로 유도해줘”
- “내가 작성한 코드에서 위험한 부분을 리뷰해줘”
...
- “수정/삭제까지 한 번에 붙여줘”
- “이 프로젝트 완성해줘”
- “에러만 고쳐줘”라고 하고 수정 내용을 이해하지 않는 방식

## 다음 개선 순서

1. 현재 코드 흐름을 찬빈이 직접 설명한다.
   - 설명하지 못하는 부분은 작은 함수 단위로 다시 구현한다.
...
찬빈에게 다음처럼 목표를 다시 잡아주는 것이 좋다.

```text
지금은 완성보다 이해가 먼저다.
AI가 만든 코드를 네 코드로 만들려면,
다음 기능을 붙이기 전에 현재 흐름을 네 말로 설명해야 한다.
오늘 목표는 새 기능 구현이 아니라 로그인, 캘린더 조회, 작성 저장 흐름을 설명하고
작은 함수부터 직접 다시 만져보는 것이다.
```

찬빈이 설명 세션을 통과한 뒤에만 다음 메시지로 넘어간다.
