...
const browser = await puppeteer.launch({
  ...
  args: [
    ...
    '--disable-gpu', '--headless', '--force-device-scale-factor=2',
  ],
...
// Block Google Fonts (they fail anyway)
...
// Inject font fallback before load
await page.evaluateOnNewDocument(() => {
  document.addEventListener('DOMContentLoaded', () => {
    const s = document.createElement('style');
    s.textContent = `* { -webkit-font-smoothing: antialiased; font-rendering: optimizeLegibility; }`;
