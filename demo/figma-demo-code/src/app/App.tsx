...
// ── 타입 ────────────────────────────────────────────────────────────────────
type Screen = "clusters" | "overview" | "service" | "service-detail" | "self-healing"
              ...
              return (
                <button key={svc.id} onClick={() => onSelect(svc.id)}
                  ...
                  <div className="flex items-center gap-1.5 mb-2.5">
                    ...
                    {!isCritical && !isWarning && <span className="px-1.5 py-0.5 bg-emerald-100 text-emerald-700 text-[8px] font-semibold rounded" style={MONO}>HEALTHY</span>}
                    {svc.issue && <span className="px-1.5 py-0.5 bg-slate-100 text-slate-500 text-[8px] rounded" style={MONO}>{svc.issue}</span>}
                  </div>
      ...
      <div className="p-5 space-y-4">
        ...
        {isCritical && (
          <div className="bg-red-600 rounded-xl p-4 flex items-center gap-3 shadow">
            ...
            <button onClick={onDiagnosticAI}
              className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 bg-white text-red-600 rounded-lg text-[11px] font-bold cursor-pointer hover:bg-red-50 transition-colors" style={SANS}>
              원인 분석 <ArrowRight size={12} />
            </button>
          </div>
        )}
...
export default function App() {
  ...
  function selectService(id: number) {
    ...
    if (svc && !svc.healthy) openDiagnosticAI();
  }
  ...
  return (
    <div className="h-screen flex overflow-hidden bg-white" style={SANS}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {screen === "service"         && (
          <ServiceGrid onSelect={selectService} onNewAI={openNewAI} />
        )}
