...
// 생성 시각 등 보조 설명: 소스 개수 + 갱신 시각(상대 표기 단순화).
function artifactDetail(artifact: Artifact): string {
  const count = artifact.source_ids.length;
  const scope = count > 0 ? `소스 ${count}개` : "전체 소스";
  return artifact.type === "note" ? "메모" : scope;
}
...
export function StudioPanel({
  ...
  const generatingType = useWorkspace((s) => s.generatingType);
        ...
        <div>
          ...
          <div className="mt-2 space-y-1">
            {artifactsLoading && artifacts.length === 0 ? (
              <div className="grid place-items-center rounded-xl border border-dashed border-border px-3 py-5 text-muted-foreground">
                <Icon name="progress_activity" size={18} className="animate-spin" />
              </div>
            ) : artifacts.length > 0 ? (
...
// 콤팩트 항목 행. 클릭 시 가운데 뷰어로 열고, hover 시 우측에 휴지통(삭제) 노출.
